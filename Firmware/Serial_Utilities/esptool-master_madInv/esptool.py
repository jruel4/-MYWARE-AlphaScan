...
class ESPROM:
    ...
    def command(self, op=None, data=None, chk=0):
        if op:
            ...
            print("sending command: "+ binascii.hexlify(pkt))
    ...
    def receive_response(self):
        ...
        print("Attempting to read head of packet")
        if self._port.read(1) != '\xc0':
            print("Number of bytes in waiting: " + str(self._port.in_waiting))
    ...
    def connect(self):
        ...
        for _ in xrange(4):
            ...
            self._port.setDTR(False) # GPIO0|-->LOW
            self._port.setRTS(True)  # CH_PD/EN|-->HIGH
            time.sleep(0.05)
            self._port.setDTR(True)  # GPIO0|-->HIGH
            self._port.setRTS(False) # CH_PD/EN|-->LOW
            time.sleep(0.05)
            self._port.setDTR(False) # GPIO0|-->LOW
            
            #print("Reintroducing flow control")
            print("omitting flow control")
    ...
    def read_reg(self, addr):
        res = self.command(ESPROM.ESP_READ_REG, struct.pack('<I', addr)) # <I is little-endian integer
        if res[1] != "\0\0":
    ...
    def flash_begin(self, size, offset):
        ...
        print("Erasing {} bytes?".format(erase_size))        
    ...
    def flash_finish(self, reboot=False):
        ...
        self.in_bootloader = False
        print("Successfully left flash mode, MCU now running")
...
def main():
    ...
    # Add subparser object for command organization
    ...
    ###########################################################################
    ###########################################################################
    # Load ram command parser
    parser_load_ram = subparsers.add_parser(
        'load_ram',
        help='Download an image to RAM and execute')
        
    # Firmware image positional argument for load ram
    ...
    # Address positional arg for mem dump
    parser_dump_mem.add_argument('address', help='Base address', type=arg_auto_int)
    
    # Size positional arg for mem dump
    parser_dump_mem.add_argument('size', help='Size of region to dump', type=arg_auto_int)
    
    # Filename positional size for mem dump
    parser_dump_mem.add_argument('filename', help='Name of binary dump')

    # Read memory command parser
    parser_read_mem = subparsers.add_parser(
        'read_mem',
        help='Read arbitrary memory location')
    
    # Address positional argument for read mem
    parser_read_mem.add_argument('address', help='Address to read', type=arg_auto_int)

    # Write mem command parser
    ...
    operation_func = globals()[args.operation] # This return a function object
    
    operation_args,_,_,_ = inspect.getargspec(operation_func) # returns the arguments that operation_func takes
    
    if len(operation_args) == 2:  # operation function takes an ESPROM connection object
    
        # TODO could use a valid ports prompt before this point
        esp = ESPROM(args.port, args.baud) #opens serial ports eg serial.Serial() and sets baud
        
        esp.connect() # NOTE: bad connection will crash here
